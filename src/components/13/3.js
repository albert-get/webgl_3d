...
function Index(){
    ...
    function init(){
        let engine=new BABYLON.Engine(canvas.current)
        function createScene(){
            ...
            let light=new BABYLON.HemisphericLight('light',new BABYLON.Vector3(30,30,20),scene)
            light.intensity=0.7

            let meshArray=[]
            meshArray.push(BABYLON.MeshBuilder.CreateBox('box',{height:6,width:6,depth:6},scene))
            meshArray.push(BABYLON.MeshBuilder.CreateSphere('shpere',{diameter:5},scene))
            ...
            let ground=BABYLON.MeshBuilder.CreateGround('gd',{width:40,height:15,subdivisions:4},scene)
            ground.material=groundMaterial
            let meshMaterial=new BABYLON.StandardMaterial('meshMaterial',scene)
            for(let i=0;i<meshArray.length;i++){
                meshArray[i].material=meshMaterial
                meshArray[i].material.diffuseColor=new BABYLON.Color3(Math.random()*16,Math.random()*16,Math.random()*16)
